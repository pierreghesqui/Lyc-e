...
def trajectoireDuChat(listV,potion='potion_1',factorMultiplier=1) : 
    ...
    crocoAvant=0
    vitesse = 4
    listLCroco = np.loadtxt("fonctions\listLCroco.txt", dtype=int) #Liste des positions du croco (lignes)
    listCCroco=np.loadtxt("fonctions\listCCroco.txt", dtype=int) #Liste des positions du croco (Colonnes)
    ...
    #Creation de l'image
    img0 = cv2.imread('img\img.png')/255    
    pygame.mixer.init()     
    pygame.mixer.music.load("musique\musicMario.mp3")   # chargement de la musique
        ...
        for k in range(nbDeplacement):
            ...
            if 65<objetLigne and objetLigne<80 and 420<objetColonne and objetColonne<430 and potionMagique==0:
                if potion =='potion_1' :
                    ...
                    sonGameOver= pygame.mixer.Sound("musique/bonusMario.mp3")
                    sonGameOver.play()   
